monitor.controller('oilManageCtrl', function ($scope, $state, $stateParams, ngDialog, $interval
    ...
    $scope.reset = function () {
		...
		$scope.params.queryOilObj.supplycompany = ""
    }
    $scope.modelreset = function () {
        ...
        $scope.params.addOilObj.supplycompany = "";
    }
                ...
                $scope.sure = function () {
                    OilInsert.save({
                        oil: {
							...
							supplycompany:$scope.params.addOilObj.supplycompany,
                        }
                    }, function (data) {
                        if (data.status == 2000) {
                            $scope.closeThisDialog();
            ...
            controller: ['$scope', '$interval', function ($scope, $interval) {
                ...
                FindByIndocnoOil.save({
                    ...
                    if (response.status == 2000) {
                            ...
                            $scope.$parent.params.addOilObj.supplycompany = response.data.supplycompany;
                    } else {
                        alert(response.msg);
                    }
                })
